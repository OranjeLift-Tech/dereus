...
class Ctx:
    ...
    def bereikbaar(self, klasse="bereikbaar", tijden_id=None):
        import navigatie
        return navigatie.bereikbaar_html(self, klasse, tijden_id)
    ...
    def whatsapp(self, klasse="wa-link", tekst="WhatsApp", glyph="whatsapp-wit"):
        """WhatsApp-link met het officiële glyph.

        De kleur van het glyph zit in het pad en niet in currentColor, want anders trekken de
        blokstijlen het merk in de kleuren van De Reus. De keuze moet dus hier gemaakt worden,
        en hij volgt de ondergrond: wit glyph op een gevulde groene knop, groen glyph op een
        lichte ondergrond. Beide zijn merkkleuren van WhatsApp.

        Sinds .wa-link overal een groene knop is, is wit de regel. De enige uitzondering is de
        mobiele belbalk, waar de knop juist een witte tegel is; die vraagt in navigatie.py
        expliciet om glyph="whatsapp".
        """
